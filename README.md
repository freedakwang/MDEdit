...
> When entering a base_url (e.g. `https://api.openai.com/v1`), it will auto-complete to the full chat completions endpoint.
...
Three action buttons appear below each AI response:
...
- Supports dragging entire folders (auto-detects MD files and images inside)
...
| **Word (.docx)** | Preserve headings, lists, and document structure |
...
> You can also drag and drop PDF / Word / Excel / PPT files onto the page for automatic detection and import.
...
| **DOCX** | Word format, natively supported for editing in WPS / Office |
| **PDF** | Save as PDF via the browser's print function |
...
- Click the **"Search"** button in the toolbar or use `⌘/Ctrl + F`
- Search all locally stored documents
...
| `⌘/Ctrl + B` | Bold |
| `⌘/Ctrl + I` | Italic |
| `⌘/Ctrl + F` | Search document library |
| `Tab` | Indent |
| `Shift + Tab` | Unindent |
...
MDEdit 是一款基于浏览器的 Markdown 编辑器，无需安装，打开 HTML 文件即可使用。编辑器内置 AI 助手，支持智能创建文档、续写内容、润色优化、翻译、生成摘要和大纲，兼容 OpenAI、DeepSeek、Claude 等主流模型 API，通过流式输出实时显示 AI 回复。编辑器采用双栏布局，左侧编写 Markdown 源码，右侧实时预览渲染效果，面板宽度可拖拽调整，双击分割条一键重置，还能切换左右位置。内置完整的格式工具栏，支持标题、粗体、斜体、链接、图片、表格、代码块、任务列表等常用格式一键插入；列表编辑时按回车自动续行，有序列表序号自动递增。支持从 PDF、Word、Excel、PPT 导入并转换为 Markdown，也可将文档导出为 HTML、DOCX、PDF 或直接复制富文本粘贴到 WPS/Office 中。编辑器提供暗色/亮色模式切换，代码高亮主题自动跟随；大纲面板可从标题自动生成层级目录，点击即可定位。文档每 5 秒自动保存草稿，下次打开自动恢复；所有打开过的文件存入本地文档库，支持全文搜索。状态栏实时显示字数、行数、预估阅读时间和光标位置。支持拖放文件和文件夹打开，自动匹配本地图片路径。整个编辑器为单个 HTML 文件，依赖通过 CDN 加载，轻量便携，适合写作、笔记和文档编辑场景。
...
> 输入 base_url（如 `https://api.openai.com/v1`）时会自动补全为完整的 chat completions 地址。
...
> 也可直接拖放 PDF / Word / Excel / PPT 文件到页面中，自动识别并导入。
...
| **DOCX** | Word 格式，WPS / Office 原生支持编辑 |
| **PDF** | 通过浏览器打印功能另存为 PDF |
